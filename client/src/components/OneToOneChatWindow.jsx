...
function OneToOneChatWindow({ chatId, avatar, name, presence}) {
  const { oneToOneChatMessages } = useSelector((state) => state.chatWindow)
  const { isChatOpen } = useSelector((state) => state.modelopen)
  const { userinfo,  oneToOneChatsPresence, oneToOneChatsinfo } = useSelector((state) => state.auth)
  ...
  const handleclick = (e) => {
    e.preventDefault()
    setText('')
    socket.emit("sendOneToOneChatMessage", { oneToOneChatId: chatId, sender: userinfo?.user?._id, text: text })
  }


  useEffect(() => {
    console.log("called from one tot one chat window", oneToOneChatMessages, chatId, avatar, name)
    const div = chatRef.current;
    if (!div) return;
   //scrollBottomOnceRef.current && 
    if (isChatOpen) {
    const chat = oneToOneChatsinfo.find((item) => item._id === chatId)
    const unread = chat?.unread?.[0]?.count
      if(unread){
      const otherUserMessages = oneToOneChatMessages.filter((msg) => msg.sender?._id !== userinfo?.user?._id)
        ...
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            if (!seenby.includes(userinfo?.user?._id)) {
              ...
              socket.emit('seenOneToOneMessages', { oneToOneChatId: chatId, oneToOneChatMsgId: msgId, userId: userinfo?.user?._id })
            }
          }
        });
      },
    ...
    <div class='w-full h-full flex flex-col'>
      <div class='p-3 flex flex-row gap-3 border-b border-slate-300 bg-white'>
        ...
        <div class='relative flex items-center '>
          ...
          {oneToOneChatsPresence?.[chatId]==='online' && <span class='bg-green-500 h-2.5 w-2.5 rounded-full absolute bottom-1.5 -right-0.5'></span>}
        </div>
        <div>
          <p class='font-medium' >{name}</p>
          <p class='text-xs text-slate-500'>{oneToOneChatsPresence?.[chatId]==='online' ?'online':'offline'}</p>
        </div>
      </div>
      {oneToOneChatMessages ?
        <div ref={chatRef} class='bg-slate-100 flex-1 p-4 flex flex-col gap-2 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-300'>
          {
            oneToOneChatMessages.map((item, index) => {
              //console.log('userinfo from onetoone chat window', userinfo)
              return (
                (userinfo?.user?._id === item?.sender?._id) ? (
                  <div  class='flex w-full justify-end'>
                    <div  class='px-3 py-2 max-w-[min(78%,24rem)] sm:max-w-[min(50%,22rem)]  bg-blue-600 rounded-2xl rounded-br-sm text-white'>
                      ...
                      <p className=' text-xs mt-1 uppercase text-right text-slate-200'>{item?.status}</p>
                    </div>
                  </div>) : (
                  <div class='flex w-full  justify-start gap-2'>
                    <div><img class='w-8 h-8 rounded-full object-cover' src={item?.sender?.profilePic} alt="" /> </div>
                    <div data-msgid={item?._id}   data-chatid={chatId} data-seenby={JSON.stringify(item?.seenBy)} ref={(el) => { if (el) { observer?.current?.observe(el) } }} class='px-3 py-2 max-w-[min(78%,24rem)] sm:max-w-[min(50%,22rem)] bg-white rounded-2xl rounded-bl-sm text-slate-800'>
